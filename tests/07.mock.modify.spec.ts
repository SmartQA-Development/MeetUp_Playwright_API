...
test('MOCK - MODIFY - Assignment - Modify driver data response for multiple drivers', async ({ page }) => {
    // Mock the api call before navigating
    await page.route('', async (route) => {

        // create mock data to be added


        // Catching the actual response
        const response = await route.fetch();
        const json = await response.json();

        //add created mock data to the original reponse
...
test('MOCK - MODIFY - ANSWER - Modify driver data response for multiple drivers', async ({ page }) => {
    // Mock the api call before navigating
